...
class deepLSTMN(Recurrent):
    ...
    def __init__(self, output_dim,
                 ...
                 dropout_W=0., dropout_U=0.,L=0, **kwargs):
        ...
        super(deepLSTMN, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        self.u = self.init((self.output_dim, 1),
        ...
        self.W_gamma = self.init((self.output_dim, self.output_dim),
                             name='{}_W_gamma'.format(self.name))
        ...
        self.W_gamma_tilde = self.init((self.output_dim, self.output_dim),
                             name='{}_W_gamma_tilde'.format(self.name))
        ...
        self.trainable_weights = [self.W_i, self.U_i, self.b_i,
                                  ...
                                  self.W_h, self.W_x, self.W_h_tilde,
                                  self.W_gamma_tilde,self.W_gamma,self.u, self.v]
    ...
    def preprocess_input(self, x):
        self.Gamma = x[:,:K.params['xmaxlen'],:]
        self.Alpha = x[:,K.params['xmaxlen']:2*K.params['xmaxlen'],:]
        x = x[:,2*K.params['xmaxlen']:,:]
        ...
        if self.consume_less == 'cpu':
            ...
            return K.concatenate([x_i, x_f, x_c, x_o, x_r ,x], axis=2)
        else:
            return x

    def step(self, x, states):
        h_tild_tm1 = states[0]
        gamma_tild_tm1 = states[1]
        ...
        gsum1 = K.dot(self.Gamma,self.W_gamma)
        gsum2 = K.dot(K.repeat_elements(x_new.dimshuffle((0,'x',1)),K.params['xmaxlen'], axis=1), self.W_x)
        gsum3 = K.dot(K.repeat_elements(gamma_tild_tm1.dimshuffle((0,'x',1)),K.params['xmaxlen'], axis=1), self.W_gamma_tilde)

        gtanhed_sum = K.tanh(gsum1 + gsum2 + gsum3) 
        b_t = K.dot(gtanhed_sum, self.u)[:,:,0]
        p_t = K.softmax(b_t)
        gamma_tilde_t = T.batched_dot(self.Gamma.dimshuffle((0,2,1)), p_t.dimshuffle((0,1,'x')))[:,:,0]
        alpha_tilde_t = T.batched_dot(self.Alpha.dimshuffle((0,2,1)), p_t.dimshuffle((0,1,'x')))[:,:,0]
        ...
        r = self.inner_activation(x_r + K.dot(gamma_tilde_t * B_U[0], self.U_r))
        o = self.inner_activation(x_o + K.dot(h_tilde_t * B_U[3], self.U_o))
        
        c_t = f * c_tilde_t + i * self.activation(x_c + K.dot(h_tilde_t * B_U[2], self.U_c)) + r * alpha_tilde_t
        ...
        return h_t, [h_tilde_t,gamma_tilde_t]
